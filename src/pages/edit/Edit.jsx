...
export default function Edit() {
    ...
    useEffect(() => {
        fetch(BACKEND_BASE_URL + `/api/docs/${docId}`, {
            method: 'GET', headers: {
    ...
    useEffect(() => {
        ...
        fetch(BACKEND_BASE_URL + `/api/docs/changes/${docId}`, {
            method: 'GET', headers: {
                "Authorization": localStorage.getItem('jwtKey')
            },
        }).then(res => res.json()).then(data => {
    ...
    return (<>
        {loading && <div className="flex h-screen w-screen items-center justify-center bg-slate-100">
            <img src={loadingGif} className="m-auto"/>
        </div>}
        {!loading && <>
            <NavBar title={state} signedin={loading} setsignedin={setLoading} usernames={currentUsers} showBackButton/>

            <div className="flex min-h-screen justify-center bg-slate-100 px-3 py-6 md:px-6">
                <div className="w-full max-w-4xl text-black">
                    <div id="toolbar" className='flex justify-center rounded-t-xl border border-slate-200 bg-white'>
